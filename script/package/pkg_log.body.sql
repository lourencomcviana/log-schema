...
--FORMAT adotado para o timestamp da pk do log
  ...
  function is_number(p_xml in xmltype, p_path in varchar2)
    return int is
    v_xmltemp xmltype := p_xml.extract(p_path);
    begin

      if (v_xmltemp is not null)
      then
  ...
  FUNCTION fnc_extract_ignore(LOG_REFERENCE xmltype)
    return NUMBER IS
    BEGIN
      IF (LOG_REFERENCE is not null and LOG_REFERENCE.EXTRACT('/log/@ignore') IS NOT NULL)
      THEN
  ...
  FUNCTION fnc_extract_seq(LOG_REFERENCE xmltype)
    return NUMBER IS
    BEGIN
      IF (LOG_REFERENCE is not null and LOG_REFERENCE.EXTRACT('/log/@seq') IS NOT NULL)
      THEN
  ...
  FUNCTION fnc_extract_update(LOG_REFERENCE xmltype)
    return NUMBER IS
    BEGIN
      IF (LOG_REFERENCE is not null and LOG_REFERENCE.EXTRACT('/log/@update') IS NOT NULL)
      THEN
  ...
  FUNCTION fnc_extract_data(LOG_REFERENCE xmltype)
    return timestamp IS
    BEGIN
      IF (LOG_REFERENCE is not null and LOG_REFERENCE.EXTRACT('/log/@data') IS NOT NULL)
      THEN
  ...
  FUNCTION fnc_generate_time_key(prefix varchar2, precision_size number, maxsize number)
    return varchar2 IS
    T_key   varchar2(20);
  ...
  FUNCTION F_TO_LOG_DATA(p_data_str in varchar2)
    return timestamp is
    BEGIN
  ...
  FUNCTION F_METADATA_EXTRACT(p_xml in XMLTYPE)
    return t_logkey_recordtype
  pipelined is
    t_logkey r_logkey_recordtype;
    BEGIN


      if (is_number(p_xml, '/log/@seq') = 1)
      then
        t_logkey.seq := p_xml.extract('/log/@seq').getnumberval;
      end if;

      if (p_xml.EXTRACT('/log/@data') is not null)
      then
        t_logkey.data_log := F_TO_LOG_DATA(p_xml.EXTRACT('/log/@data').getstringval());
      end if;

      if (is_number(p_xml, '/log/@ignore') = 1)
      then
  ...
  FUNCTION F_CONTEXT_LEAF(p_ID_CONTEXT NUMBER)
    return T_CONTEXT_NODEtree_recordtype
  pipelined is
  ...
  FUNCTION F_METADATA_BASIC(p_xml in XMLTYPE)
    return XMLTYPE is
    t_saida xmltype;
    BEGIN
  ...
  FUNCTION F_XML_LAST_UPDATE(p_xml in XMLTYPE)
    return XMLTYPE is
    BEGIN
      return F_XML_LAST_UPDATES(0, p_xml);
    end;

  FUNCTION F_XML_LAST_UPDATES(p_number_updates number, p_xml in XMLTYPE)
    return XMLTYPE is
    newXml xmltype;
    BEGIN
    ...
    BEGIN
      ...
      BEGIN
        ...
        FROM (
               with cte as (
                   select *
                   from TABLE (LOG_AUDIT.PKG_UTIL.F_SPLIT(TEXT))
               )
               SELECT
                 C.CONTEXT,
                 level                                           as node_level,
          ...
          BEGIN
            ...
            WHERE NTREE.node_level = OTREE.node_level AND NTREE.cONTEXT = OTREE.CONTEXT AND
                  (T_ID_CONTEXT_FATHER IS NULL OR T_ID_CONTEXT_FATHER = ID_CONTEXT_FATHER);

            IF (NTREE.DEFAULT_PRIORITY_LEVEL is not null)
            THEN
  ...
  PROCEDURE P_ADD_PRIVATE(p_name          IN OUT VARCHAR2,
    ...
    BEGIN
      ...
      if (fnc_extract_ignore(LOG_REFERENCE) = 0)
      then
      ...
      THEN
        ...
        then
          ...
          BEGIN
            ...
            if (LOG_REFERENCE is not null)
            then
              P_ADD_PRIVATE(T_name2, LOG_REFERENCE.getClobVal(), t_log_erro, dup_error_count + 1, o_data, o_seq,
                            o_update);
            else
  ...
  PROCEDURE P_LOG_PRIVATE(P_Mensagem IN CLOB, LOG_REFERENCE IN OUT XMLTYPE) AS
    ...
    BEGIN
      ...
      if (length(p_Mensagem) > 200)
      then
        t_mensagem := SUBSTR(P_Mensagem,190)||'...';
      else
    ...
    '
      ...
      if (length(p_Mensagem) > 200) then
          P_add_PRIVATE(t_mensagem, p_Mensagem, LOG_REFERENCE);
      end if;
  ...
  PROCEDURE P_CONTEXT_PRIORITY(P_ID_PRIORITY IN NUMBER, p_CONTEXT in varchar2, LOG_REFERENCE in OUT XMLTYPE) AS
    ...
    BEGIN
      ...
      THEN
        ...
        THEN
          ...
          THEN
            ...
            then
              ...
              select xmlelement("log", xmlattributes (T_DATA_CHAR as "data", T_seq as "seq"))
              into LOG_REFERENCE
              from dual;
            end if;
          else
        ...
        THEN
          T_DATA_CHAR := to_char(T_data, C_DATA_LOG_FORMAT);
          select xmlelement("log", xmlattributes (T_DATA_CHAR as "data", T_seq as "seq", '1' as "ignore"))
          into LOG_REFERENCE
          from dual;
        END IF;
  ...
  PROCEDURE P_LOG_ERR(P_Mensagem        IN     VARCHAR2, P_CONTEXT IN Varchar2, PARENT_LOG_REFERENCE IN xmltype,
                      ERR_LOG_REFERENCE IN OUT xmltype) AS
    ...
    BEGIN
      ...
      p_add('BACKTRACE', REGEXP_REPLACE(DBMS_UTILITY.FORMAT_ERROR_BACKTRACE, T_REMOVE_SPACE_REGEX, ''),
            ERR_LOG_REFERENCE);
      p_add('CALL_STACK', REGEXP_REPLACE(DBMS_UTILITY.FORMAT_CALL_STACK, T_REMOVE_SPACE_REGEX, ''), ERR_LOG_REFERENCE);

      IF (PARENT_LOG_REFERENCE IS NOT NULL)
      then
  ...
  PROCEDURE P_ADD(p_name IN VARCHAR2, p_Value IN CLOB, LOG_REFERENCE in OUT XMLTYPE) AS
    ...
    BEGIN
      ...
      IF (p_Value IS NOT NULL AND fnc_extract_ignore(LOG_REFERENCE) = 0)
      THEN
  ...
  PROCEDURE P_ADD(p_name IN VARCHAR2, p_Value IN XMLTYPE, LOG_REFERENCE in OUT XMLTYPE) AS
    ...
    BEGIN
      ...
      IF (P_Value.EXISTSNODE('/log/update/parameter') = 1)
      THEN
        BEGIN
        ...
        THEN
          begin
            ...
            then
              ...
              begin
                if (SQLCODE = -19010)
                then --there is no root node, need a root node
  ...
  PROCEDURE P_IMPORT_PARAMETER(p_PARAMETERS_XML IN XMLTYPE, LOG_REFERENCE in OUT XMLTYPE) AS
    BEGIN
      ...
      FROM
        ...
        begin
          IF (C_PARAMETER.dataType = 1)
          THEN
            P_ADD(C_PARAMETER.name, C_PARAMETER.Value.extract('/parameter/*'), LOG_REFERENCE);
          ELSIF (C_PARAMETER.dataType = 2)
            THEN
              P_ADD(C_PARAMETER.name, C_PARAMETER.Value.extract('/parameter/text()').getnumberVal(), LOG_REFERENCE);
          ELSIF (C_PARAMETER.dataType = 3)
            THEN
              P_ADD(C_PARAMETER.name,
                    TO_DATE(C_PARAMETER.Value.extract('/parameter/text()').getstringval(), 'yyyy-mm-dd"T"hh:mi:ss'),
                    LOG_REFERENCE);
          ELSE
